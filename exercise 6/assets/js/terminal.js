...
const clear = (input, textarea, filesystem) => {
    if (input.length > 1) {
...
const localStorage = (input, filesystem) => {
    if (input.length > 1) {
        return "Errore: input non valido";
    }
    if (input.length === 0) {
...
const change_dir = (input, bar_text, filesystem) => {
...
function invio(event, text, bar_text, filesystem) {
    if (event.keyCode === 13) {
        ...
        if (array.length > 0) {
            switch (array[0]) {
                ...
                case "echo":
                    ...
                    break;
                case "clear":
                    _clear = clear(array, text, filesystem);
                    break;
                case "ls":
                    text.value += localStorage(array.slice(1), filesystem) + "\n";
                    break;
                case "cd":
                    text.value += change_dir(array.slice(1), bar_text, filesystem) + "\n";
                    break;
                case "pwd":
                    text.value += filesystem.getName() + "\n";
                    break;
                case "mkdir":
                    text.value += mkdir(array.slice(1), filesystem) + "\n";
                    break;
                case "touch":
                    text.value += touch(array.slice(1), filesystem) + "\n";
                    break;
                case "rm":
